...
# ---------------------------------------------------------------------------
{
    my $commit = QBitcoin::Downgrade->new({
        ...
        btc_txid => $txid, btc_vout => 0, btc_value => 50_000, scriptpubkey => $spk,
    });
    ...
    is($tx->down->freeze_txid,  $freeze_txid, "commitment freeze_txid");
    is($tx->down->freeze_vout,  $freeze_vout, "commitment freeze_vout");
...
# ---------------------------------------------------------------------------
{
    ...
    QBitcoin::Downgrade::Commitment->create({
        tx_id => 1, freeze_txid => $freeze_txid, freeze_vout => $freeze_vout,
        btc_txid => $txid, btc_vout => 2, btc_value => 99, scriptpubkey => $spk,
    });
    my ($row) = QBitcoin::Downgrade::Commitment->find(tx_id => 1);
    ok($row, "commitment stored and fetched");
    is($row->freeze_txid,  $freeze_txid, "stored commitment freeze_txid round-trips");
    is($row->freeze_vout,  $freeze_vout, "stored commitment freeze_vout round-trips");
